...
tmp102 = api.feeds.get(FeedId1)
bmp085 = api.feeds.get(FeedId2)


b = [bmp.readTemperature(),float(bmp.readPressure())/1000, bmp.readAltitude()]
t = tmp.readTemperature()
    
time = datetime.datetime.now()
bmp085.datastreams= [xively.Datastream('Temperature',current_value=b[0],at=time),
                     xively.Datastream('Pressure',current_value=b[1],at=time),
                     xively.Datastream('Altitude',current_value=b[2],at=time)]
bmp085.update()
    
    
tmp102.datastreams = [xively.Datastream('TMP102',current_value=t[1],at=time)]
tmp102.update()
